...
## 8. Cotrans/or Xianscan Porting Strategy
When adapting code or logic from Cotrans (Python) or XianScan (Rust) to this project (TypeScript), you MUST ALWAYS look at the actual Cotrans/ Xianscan codebase before making any changes or proposing solutions. Do not invent your own math or logic if Cotrans already solves it.
both Donors repo leave in reference folder
...
- **Documenting New Workarounds:** If you exhaust standard solutions and must implement a non-obvious workaround:
  ...
  2. Document the detailed architecture, root cause, and affected modules in [`docs/development/workarounds.md`](file:///Users/dangtruongan/study/Kites/docs/development/workarounds.md) or corresponding devlogs, keeping this file short and concise.

### Key Architectural Invariants & Reference Links
Complete technical documentation and rationale for all mitigations live in [`docs/development/workarounds.md`](file:///Users/dangtruongan/study/Kites/docs/development/workarounds.md):

* **Vite 8 & CRXJS 2.7.1**: Keep `build.rollupOptions` (do not rename to `rolldownOptions`), retain explicit `optimizeDeps.entries`, exclude native packages (`ppu-paddle-ocr`, `canvas`, `onnxruntime-node`), and keep mode-aware minification. Specs: [`docs/development/workarounds.md#1-vite-8--crxjs-271-build-configuration`](file:///Users/dangtruongan/study/Kites/docs/development/workarounds.md#1-vite-8--crxjs-271-build-configuration) · [`vite.config.ts`](file:///Users/dangtruongan/study/Kites/vite.config.ts) · [Devlog 013](file:///Users/dangtruongan/study/Kites/docs/dev-history/devlog/013_phase13_dev_instrumentation_and_vite8_crxjs_compatibility.md).
* **Runtime Messaging & WebGPU**: Explicitly address offscreen RPCs (`target: 'background'`, `source`, `request: true`). Serialize popup writes sequentially and normalize at background boundary. Probe WebGPU only offscreen with high performance. Specs: [`docs/development/workarounds.md#2-chrome-extension-runtime-messaging-popup-state-and-webgpu-invariants`](file:///Users/dangtruongan/study/Kites/docs/development/workarounds.md#2-chrome-extension-runtime-messaging-popup-state-and-webgpu-invariants) · [Devlog 015](file:///Users/dangtruongan/study/Kites/docs/dev-history/devlog/015_phase15_offscreen_webgpu_detection_and_lama_recovery.md).
* **SPA Image Replacement**: Always clear `srcset` when updating `src`, convert base64 to same-origin blob URLs, and attach reversion shields with `isSelfMutating` flag against SPA re-renders. Specs: [`docs/development/workarounds.md#3-spa--responsive-image-replacement-strategy-xianscan-reference-pattern`](file:///Users/dangtruongan/study/Kites/docs/development/workarounds.md#3-spa--responsive-image-replacement-strategy-xianscan-reference-pattern) · [`src/content/index.tsx`](file:///Users/dangtruongan/study/Kites/src/content/index.tsx) · [Devlog 014](file:///Users/dangtruongan/study/Kites/docs/dev-history/devlog/014_phase14_spa_image_replacement_and_xianscan_pattern.md).
* **Neural Inpainting Architecture**: Always inpaint localized patches snapped to 64px buckets; never run full canvas or squish to 512px. Expand polygon masks with $\ge 4$px round stroke (`UNCLIP_RATIO = 1.8`). Specs: [`docs/development/workarounds.md#4-neural-inpainting-architecture-lama--xianscan-dynamic-patch-strategy`](file:///Users/dangtruongan/study/Kites/docs/development/workarounds.md#4-neural-inpainting-architecture-lama--xianscan-dynamic-patch-strategy) · [`src/offscreen/engines/inpaint/LamaMangaInpaintEngine.ts`](file:///Users/dangtruongan/study/Kites/src/offscreen/engines/inpaint/LamaMangaInpaintEngine.ts) · [Devlog 016](file:///Users/dangtruongan/study/Kites/docs/dev-history/devlog/016_phase16_xianscan_dynamic_patch_inpainting_and_lama_quality_recovery.md).
