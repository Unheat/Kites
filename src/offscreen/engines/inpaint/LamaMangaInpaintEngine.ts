...
/**
 * Tier 4 Inpainting Engine: LaMa Manga.
 * High-Speed 1:1 Localized Dynamic Patch Inpainting Engine.
 ...
 * Instead of computing full-page convolutions across 2.3M pixels or squishing large
 * clusters down to 512x512, it crops tight 1:1 native resolution patches around each
 * speech bubble (e.g. 192x256), snaps dimensions to 64px boundaries to prevent GPU
 * shader recompilations, and executes localized neural inpainting. This delivers
 * sub-second execution speeds with 100% native resolution sharpness and zero blur.
 */
export class LamaMangaInpaintEngine extends LamaBaseInpaintEngine {
  ...
  /**
   * Returns the local filesystem path to the dynamic manga LaMa ONNX binary (used in Node.js test environment).
   *
   ...
   *
   * @param value - Model output pixel float in [0.0, 1.0].
  ...
  async inpaint(
    ...
    if (!this.session) {
      throw new Error('[LamaMangaInpaintEngine] Model not initialized');
    }
    ...
    console.log(`[LamaMangaInpaintEngine] Starting localized patch inpainting for ${boxes.length} bubble components using ${this.activeProvider}.`);
    ...
    console.log(`[LamaMangaInpaintEngine] Localized patch inpainting completed in ${(endTime - startTime).toFixed(2)}ms for ${boxes.length} bubbles.`);
